...
class DiagnosisControllerrr extends Controller
{
    /**
    ...
    private function nilaiEvidence()
    {
        return [
            1 => 'Tidak Pernah',
            2 => 'Sesekali Terjadi Dalam 6 Bulan Terakhir',
            3 => 'Mungkin Sering Terjadi Dalam 6 Bulan Terakhir',
            4 => 'Hampir Sering Terjadi Setiap Hari Selama 6 Bulan Terakhir',
            5 => 'Sering Terjadi Dalam Sehari Minimal 5 Kali Selama 6 Bulan Terakhir',
        ];
    }
    ...
    public function diagnosis()
    {
        ...
        $data = [
            'gejalas' => Gejala::with('kriteria')->paginate(4),
            'riwayat' => Riwayat::find($riwayat_id),
            'nilai_evidence' => $this->nilaiEvidence(),
        ];
    ...
    public function prosesDiagnosis(Request $request)
    {
        ...
        \Log::info('Request Gejalas: ', $gejalas);
        ...
        foreach ($gejalas as $gejala) {
            ...
            $gejalas_input[$gejala->id] = $request->input('gejalas')[$gejala->id];

            if (isset($gejalas_input[$gejala->id])) {
                $nilai_user[$kriteria][$gejala->id] = floatval($gejalas_input[$gejala->id]);
            } else {
                $nilai_user[$kriteria][$gejala->id] = 0;
            }
        ...
        foreach ($nilai_pakars as $kriteria => $nilai_pakar) {
            ...
            foreach ($nilai_pakar as $id => $nilai) {
                ...
                $nilai_user_kriteria_id = isset($nilai_user[$kriteria][$id]) ? $nilai_user[$kriteria][$id] : 0.0;
                $total_sementara += $nilai_semesta[$kriteria][$id] * $nilai_user_kriteria_id;
            }
        ...
        foreach ($hasil_diagnosis as $hasil) {
            foreach ($hasil as $gejala_id => $nilai_hasil) {
                Diagnosis::where('riwayat_id', $riwayat_id)->where('gejala_id', $gejala_id)->update([
        ...
        session()->put('total_pakars', $total_pakar);
    ...
    public function kategoriHasil(Request $request)
    {
        ...
        Riwayat::where('id', $diagnosis)->update([
