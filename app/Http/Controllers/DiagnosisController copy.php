...
class DiagnosisControllerr extends Controller
{
    /**
    ...
    public function prosesDiagnosis(Request $request)
    {
        ...
        foreach ($gejalas as $gejala) {
            ...
            $nilai_pakars[$kriteria][$gejala->id] = $gejala->kriteria->bobot_prioritas;

            // Tambahkan validasi untuk memastikan kunci ada
            if (isset($request->input('gejalas')[$gejala->id])) {
                $nilai_user[$kriteria][$gejala->id] = $request->input('gejalas')[$gejala->id];
                $total_pakar[$kriteria] += $gejala->kriteria->bobot_prioritas;
            } else {
                \Log::warning("Gejala ID {$gejala->id} tidak ditemukan dalam input request.");
                continue;
            }
